function [avbetax63V,avbetay63V,len63V,sbeg63V,send63V]=meanbeta_BS_63V_HLLHC(twissfilename,beam);


% function to compute the average beta functions of all beam screens of
% type 63V in HL-LHC (take out those in high-beta region close to IP1 & 5).
...
% extension of the 53 vertical beam screen in Q1 (IR8)
...
%namesQ1_IR8
sbegQ1_ext_IR8=[sbegQ1_IR8(2)-BSlengt-IClengt sendQ1_IR8(1)+IClengt];
sendQ1_ext_IR8=sbegQ1_ext_IR8+BSlengt;
% 2 BS. total length=1.54
...
[sbegQ4_5_IR6_Q5_6_IR4,sendQ4_5_IR6_Q5_6_IR4,names,len]=extract_beamscreens(twissfilename,'MQYL?\.[456][RL][46]',6.1015);
...
% Q4 in IR1 and IR5
[sbegQ4_IR1_5,sendQ4_IR1_5,names,len]=extract_beamscreens(twissfilename,'MQYY\.4[RL][15]',8.7765);
...
%size(sbegQ3_IR8),namesQ3_IR8,sum(len),sum(sendQ3_IR8-sbegQ3_IR8)
...
% Q2 in IR8 (two quads in each)
[sbegQ2_IR5_8,sendQ2_IR5_8,names,len]=extract_beamscreens(twissfilename,{'MQXB\.A2[RL][8]','MQXB\.B2[RL][8]'},13.23);
%size(sbegQ2_IR8),names,sum(len),sum(sendQ2_IR8-sbegQ2_IR8)
% 2 BS. total BS length=26.46

% compiling all the type 63 vertical beam screens
[sbeg63V,ind]=sort([sbegQ1_ext_IR8 sbegUnd_IR4_R sbegDFBA_Q7L sbegDFBA_Q7R sbegQ4_5_IR6_Q5_6_IR4 ...
    sbegQ4_IR1_5 sbegQ3_IR8 sbegQ4_5_IR2_8 sbegQ2_IR5_8]);
send63V=[sendQ1_ext_IR8 sendUnd_IR4_R sendDFBA_Q7L sendDFBA_Q7R sendQ4_5_IR6_Q5_6_IR4 ...
    sendQ4_IR1_5 sendQ3_IR8 sendQ4_5_IR2_8 sendQ2_IR5_8];
send63V=send63V(ind);
length(sbeg63V),length(send63V) % 41=2+1+2*8+8+4+2+6+2
len63V=sum(send63V-sbeg63V) % total length: 236.7315
